...
namespace Crppy
{
    internal static class Program
    {
        ...
        private static IEnumerable<byte[]?> Iter8M(Action<A, AR8, AMO> func)
        {
            foreach (var (b, c) in new[]
                     {
                         (ah, __byte_ptr[bx])
        ...
        private static IEnumerable<byte[]?> IterSb(Action<A, sbyte> func)
        {
            foreach (var it in new sbyte[] { 0, 3, 7, 9, 13 })
                yield return E.GetBytes(a => func(a, it));
        }
        ...
        {
            var byteArrays = Generate()
                .Where(b => b != null).Cast<byte[]>();
